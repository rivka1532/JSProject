...
$.ajax({
    ...
    success: (data) => {
        if(sort == "c"){
            ...
            h2.innerHTML = `כאן תמצאו מגוון ענק של כל סוגי המתכונים. טיפים והמון מידע נוסף, מצאו את המתכון המנצח הבא שלכם!`
        }
        else {
            recipes = data.recipes.filter((recipe) => recipe.kashrut === categoryName);
            if(categoryName==="כשר פרווה"){
                h1.innerHTML = `מתכונים פרווה`;
                h2.innerHTML = `מחפשים מתכונים פרווה? כאן תמצאו מגוון ענק של מתכונים פרווה, טיפים והמון מידע נוסף, מצאו את המתכון המנצח הבא שלכם!`
            }
            else{
